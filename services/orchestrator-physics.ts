...
import { generateBaseProblem } from "./subjects/physics/generator";
import { applyTraps } from "./subjects/physics/traps/trap-master";
import { validateAndMergeTraps } from "./nodes/node4-validator";
import { solveAndFormatProblem } from "./subjects/physics/solver";
import { fuseProblemText } from "./subjects/physics/text-fusion";
...
export async function runPhysicsMultiNodeWorkflow(
        ...
        const generateSingleProblem = async (i: number): Promise<FinalProblem | null> => {
            ...
            while (!validProblem && retryCount <= maxRetries) {
                try {
                    ...
                    const baseProblem = await generateBaseProblem(input, constraints, i + 1, allowedAngles, usedAngles, i);
                    ...
                    if (!validationResult.isValid) {
                        console.warn(`[物理] 第${i+1}题 Node4 验证失败 (尝试${retryCount+1}):`, validationResult.conflicts);
                        retryCount++;
                        continue;
                    }
                    ...
                    if (!solverResult.isValid || !solverResult.formattedSolution) {
                        console.warn(`[物理] 第${i+1}题 Node5 求解失败 (尝试${retryCount+1}):`, solverResult.errorMessage);
                        retryCount++;
                        continue;
                    }
                    ...
                    if (!outputValidation.isValid) {
                        console.warn(`[物理] 第${i+1}题 Node7 输出验证失败:`, outputValidation.errors);
                        retryCount++;
                        continue;
                    }
                    ...
                    if (validProblem) {
                        (validProblem as any).subject = 'physics';
                    }

                } catch (error) {
                    console.error(`[物理] 第${i+1}题 出错 (尝试${retryCount+1}):`, error);
                    retryCount++;
                    if (retryCount > maxRetries) {
                        console.error(`[物理] 第${i+1}题 已达最大重试次数，放弃`);
                        break;
                    }
                }
            }
            ...
            if (!validProblem) {
                console.error(`⚠️ [物理] 跳过第 ${i + 1} 道题（${maxRetries} 次重试后仍失败）`);
                return null;
            }
            ...
            console.log(`[物理] 第${i+1}题 成本估算: ${costSummary.formatted} (${costSummary.callCount}次LLM调用, 模型: ${costSummary.modelsUsed.join(', ')})`);

            if (callbacks?.onProblemGenerated) {
                try {
                    await callbacks.onProblemGenerated(validProblem, i);
                } catch (saveError) {
                    console.warn(`[物理] 第${i+1}题 保存失败:`, saveError);
                }
            }
