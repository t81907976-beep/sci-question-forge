...
/**
 * Materials Science: Solver & Formatter (Standard Solution with Trap Avoidance)
 */
...
async function performSanityCheck(
    ...
    const checkPrompt = `
你是材料科学与工程审核专家。在求解以下题目前，必须独立执行防伪审查（Sanity Check），检查题目本身的材料学与物理合理性。
...
【防伪审查清单】：
0. **陷阱注入差异审查（最重要）**：逐句对比差异，检查是否引入材料学或物理条件矛盾
1. **常数合法性**：物理常数是否与标准值一致（N_A = 6.02214076×10²³ mol⁻¹, R = 8.314 J/(mol·K)）
2. **晶体结构匹配**：晶体结构与对应材料是否真实匹配（如 Cu 为 FCC、α-Fe 为 BCC、Mg 为 HCP）
3. **相变与工艺温度自洽性**：相变温度、热处理温度与相图（如 Fe-C 相图 A1=727°C）是否一致
4. **材料性能参数合理性**：弹性模量、强度、Tg 等是否处于该材料的实测量级
5. **数据完整性**：求解目标所需数据是否完整
6. **微积分与数学规范性**：偏导数与全导数是否正确区分
7. **加工与极限条件的合理性**（如加工温度不得超过熔点）
...
【输出格式】（严格 JSON，不含 markdown）：
{
  ...
  "checksPerformed": {
    ...
    "phaseTransitionConsistency": "通过/失败/无法检查",
    "materialPropertyPlausibility": "通过/失败/无法检查",
...
`;
    ...
    try {
        const content = (await callLLMTracked(checkPrompt, { model: 'review', temperature: 0.2 }, problemIndex));
...
async function attemptProblemRepair(
    ...
    const repairPrompt = `
你是材料科学与工程问题修复专家。一个题目在防伪审查中发现了以下问题：
...
`;
    ...
    try {
        const content = (await callLLMTracked(repairPrompt, { model: 'review', temperature: 0.2 }, problemIndex));
        const jsonMatch = content.match(/\{[\s\S]*\}/);
...
export async function solveAndFormatProblem(
    ...
    const prompt = hasTraps
        ? `
你是材料科学与工程问题验证与教学专家。请尝试完整解决以下题目。
...
`;
    ...
    try {
        const content = (await callLLMTracked(prompt, {
            model: 'reasoning',
            systemPrompt: "你是顶级的材料科学与工程专家。请验证并严格以 JSON 格式输出解题报告。"
        }, problemIndex));
        const jsonMatch = content.match(/\{[\s\S]*\}/);
        if (!jsonMatch) {
        ...
        console.error("Materials Science Solver Error:", error);
