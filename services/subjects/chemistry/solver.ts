...
import physicsConstantsData from "../../data/physicsConstants.json";
...
/**
 * Chemistry: Solver & Formatter (Standard Solution with Trap Avoidance)
 */
...
async function performSanityCheck(
    ...
    const checkPrompt = `
你是物理化学审核专家。在求解以下题目前，必须独立执行防伪审查（Sanity Check），检查题目本身的物理合理性。
...
async function attemptProblemRepair(
    ...
    const repairPrompt = `
你是物理化学问题修复专家。一个题目在防伪审查中发现了以下问题：
...
`;
    ...
    try {
        const content = (await callLLM(prompt, {
            model: 'reasoning',
            systemPrompt: "你是顶级的化学专家。请验证并严格以 JSON 格式输出解题报告。"
        }));
        const jsonMatch = content.match(/\{[\s\S]*\}/);
        if (!jsonMatch) {
